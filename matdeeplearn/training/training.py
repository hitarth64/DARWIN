...
def evaluate(loader, model, loss_method, rank, out=False):
    ...
    for data in loader:
        ...
        with torch.no_grad():
            output = model(data); 
            output = output.reshape(data.y.shape)
            loss = getattr(F, loss_method)(output, data.y)
...
def train_regular(
    ...
    if pretrained_model is None:
        ...
        optimizer = getattr(torch.optim, model_parameters["optimizer"])(model.parameters(),lr=model_parameters["lr"], **model_parameters["optimizer_args"])
    else:
        model = pretrained_model.to(rank)
        optimizer = getattr(torch.optim, model_parameters["optimizer"])(filter(lambda p: p.requires_grad, model.parameters()), lr=model_parameters["lr"], **model_parameters["optimizer_args"])

    scheduler = getattr(torch.optim.lr_scheduler, model_parameters["scheduler"])(optimizer, **model_parameters["scheduler_args"])
...
def tune_trainable(config, checkpoint_dir=None, data_path=None):
    ...
    for epoch in range(1, model_parameters["epochs"] + 1):
        ...
        val_error, val_out = evaluate(
            val_loader, model, training_parameters["loss"], rank=rank, out=True
        )

        trues = [int(float(val_out[i][1])) for i in range(len(val_out))]
        preds = [int(float(val_out[i][2])) for i in range(len(val_out))]

        if set(trues) == set([0,1]):
            print('F1-score on validation: ', f1_score(trues,preds))
