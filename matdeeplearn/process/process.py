...
def process_data(data_path, processed_path, processing_args):
    ...
    for index in range(0, len(target_data)):
        ...
        comp = torch.tensor(np.nan_to_num(ep.featurize(AAA.get_structure(ase_crystal).composition)), dtype=torch.float32).reshape(1,-1)
...
def NormalizeEdge(dataset, descriptor_label, feature_min=None, feature_max=None):
    ...
    if feature_min == None or feature_max == None:
