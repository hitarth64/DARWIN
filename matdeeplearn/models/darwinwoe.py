...
class AtomConv(MessagePassing):
    def __init__(self, gc_dim, dim3, aggr='add', **kwargs):
        super(AtomConv, self).__init__(aggr=aggr, **kwargs)
        nn_list = []
        for i in range(dim3):
            if i==0:
                nn_list.append(torch.nn.Linear(2*gc_dim, gc_dim))
            else:
...
# CGCNN
class darwinwoe(torch.nn.Module):
    def __init__(
        self,
        data,
        dim1=64,
