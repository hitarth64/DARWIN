...
class fCGCNN(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.comp_lin = torch.nn.Linear(132,132) # to account for scaling of feature vectors
        ...
        if post_fc_count > 0:
            ...
            for i in range(post_fc_count):
                if i == 0:
                    ...
                    if self.pool_order == "early" and self.pool == "set2set":
                        lin = torch.nn.Linear(post_fc_dim * 2+132, dim2) #132 comes from chemical feature dimension
                    else:
                        lin = torch.nn.Linear(post_fc_dim+132, dim2)
            ...
            self.post_lin_list = torch.nn.ModuleList()
            if self.pool_order == "early" and self.pool == "set2set":
                self.lin_out = torch.nn.Linear(post_fc_dim*2 +132, output_dim)
            else:
                self.lin_out = torch.nn.Linear(post_fc_dim + 132, output_dim)   
    ...
    def forward(self, data):
        ...
        if self.pool_order == "early":
            ...
            w_comp = self.comp_lin(data.comp)
            out = torch.cat([out,w_comp],axis=1)
            ...
            for i in range(0, len(self.post_lin_list)):
                out = self.post_lin_list[i](out)
                out = getattr(F, self.act)(out)
            out = torch.cat([out,data.comp],axis=1)
            out = self.lin_out(out)
            if self.pool == "set2set":
                out = self.set2set(out, data.batch)
